...
void Interpreter::interpreter(Element* element)
{
    element->accept(this);
    if (element->m_part != NULL)
    {
        interpreter(element->m_part->m_next);
    }
    if (element->m_next != NULL)
    {
        interpreter(element->m_next);
    }
}
void Interpreter::visit(Lab* element)
{
    context->m_output += values[n]*element->m_weight;
    n++;
}
void Interpreter::visit(Test* element)
{
    context->m_output += values[n]*element->m_weight;
    n++;
}
void Interpreter::visit(Midterm* element)
{
    if (element->m_part == NULL)
    {
        context->m_output += values[n]*element->m_weight;
...
void Interpreter::visit(Exam* element)
{
    if (element->m_part == NULL)
    {
        context->m_output += values[n]*element->m_weight;
...
void Interpreter::visit(Element* element)
{
    //cout << "visit Element ";
}

void Interpreter::setup(Context* c, int* v)
{
    context = c;
    values = v;
    n = 0;
